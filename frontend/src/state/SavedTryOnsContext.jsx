...
/**
 * Client-side saved try-on results (visit-scoped state).
 *
 * Holds snapshots of virtual try-on fitting room results so the customer can
 * access /account/saved-try-ons during their session.
 * Mirrors the convention of SavedDesignsContext so a future API can sync both
...
export function SavedTryOnsProvider({ children }) {
  ...
  const saveResult = useCallback((rawResult) => {
  ...
  const removeResult = useCallback((saveId) => {

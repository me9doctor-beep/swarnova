...
| Four-film rotation, one film playing at a time | **Done** — order and timing measured over a 52 s watch |
...
| `npm test` / `npm run build` | **261 / 261 pass · build ✓** (no test changes needed) |
...
* Order 1 → 2 → 3 → 4 → 1 ✓, ~10 s per film (`ended`-driven, 14 s watchdog
  unused), 1.8 s opacity-only dissolve (`heroReelDissolve`) — no carousel
  movement, no slide, no scale.
* **Max 2 `<video>` elements ever mounted** (only during the dissolve, the
  outgoing one paused on its final frame via `holdFinalFrame`); **max 1
  playing** at any moment ✓.
* MP4 network events during the watch: one request per film, each issued
  only when that film becomes active (§8) — the next film is never prefetched.
...
* **Only the active film is fetched and mounted.** The next film is not
  requested until it rotates in (measured: single 206 per film, ~10 s apart).
  During the 1.8 s dissolve two elements exist but only one is playing.
  Poster-first, `preload="auto"` for the active film / `"none"` for the
  outgoing layer — unchanged strategy, no new JS/video preload penalty.
* Production build inlines the four films as base64 data URIs
  (`vite-plugin-singlefile`, the repo's standing convention):
  `dist/index.html` **22,447 kB (gzip 16,280 kB)** — was 5.5 MB with the
  tiny placeholder clips. The production-build playback/rotation checks
  still pass (§4.1, §5). As already recorded in the Phase 14.4
  reconciliation, real deployments should serve campaign footage as CDN/DAM
  URLs through the API instead of bundling — the media contract already
  carries plain URL strings, so that remains a data-only change.

## 14. Tests

```
npm test   →  # tests 261   # pass 261   # fail 0
```

No assertion needed changing: the Phase 14.4A suite was written to accept
either state per slot — `const … = null;` **or** a real
`hero-reel/*.mp4` import — and the node test harness resolves MP4 imports
to `""`, so the poster-mode/undelivered-footage assertions still exercise
the null path while the shipped slots are real imports. No unrelated test
was touched.
...
npm run build → ✓ 2236 modules transformed · built in 7.2 s
                dist/index.html  22,447.35 kB │ gzip 16,279.83 kB
```
